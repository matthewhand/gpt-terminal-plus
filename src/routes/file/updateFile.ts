...
export const updateFile = async (req: Request, res: Response) => {
  const { directory, filename, pattern, replacement, backup = true } = req.body;
  ...
  try {
    ...
    const success = await serverHandler.updateFile(fullPath, pattern, replacement, backup);
    debug(`File updated: ${fullPath}, pattern: ${pattern}, replacement: ${replacement}, success: ${success}`);
