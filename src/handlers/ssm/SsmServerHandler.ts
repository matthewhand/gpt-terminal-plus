...
export class SsmServerHandler implements ServerHandlerInterface {
  ...
  async listFiles(directory: string = "", limit: number = 42, offset: number = 0, orderBy: "filename" | "datetime" = "filename"): Promise<PaginatedResponse<string>> {
    ...
    // Wrap the result in a PaginatedResponse
    const paginatedResponse: PaginatedResponse<string> = {
      items: result,
      totalPages: 1,  // Calculate the total number of pages if needed
      responseId: 'unique-id',  // Generate a unique ID if needed
      stdout: [''],  // Correctly initialize as an array of strings
      stderr: ['']   // Correctly initialize as an array of strings
    };
