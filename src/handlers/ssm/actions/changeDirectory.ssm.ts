...
const debug = Debug('app:ssm:changeDirectory');
...
export async function changeDirectory(directory: string): Promise<boolean> {
  try {
    ...
    // In a real SSM scenario, you would execute a command like `cd ${directory}`
    // and check its exit code. For this mock, we'll assume it always succeeds.
    debug(`Changing SSM directory to: ${directory}`);

    return true;
  } catch (err: any) {
    debug(`❌ Failed to change SSM directory: ${err.message}`);
    throw err;
  }
}
