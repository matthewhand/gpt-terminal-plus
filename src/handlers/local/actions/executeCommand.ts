...
export async function executeCommand(
    ...
    if (USE_EXECFILE) {
        ...
        // For exec, pass the command directly - no additional shell wrapping
    ...
    return new Promise((resolve, reject) => {
        if (USE_EXECFILE) {
            execFile(commandToExecute, args, execOptions, (error: any, stdout: string, stderr: string) => {
                ...
                if (error) {
                    ...
                    reject({ stdout: out, stderr: err, presentWorkingDirectory: execOptions.cwd, truncated: isMax, terminated: isMax, exitCode });
                } else {
                    resolve({ stdout, stderr, getPresentWorkingDirectory: execOptions.cwd });
                }
            });
        } else {
            exec(commandToExecute, execOptions, (error: any, stdout: string, stderr: string) => {
