...
if (!boardId || !groupId || !genreColumnId || !commentColumnId) {
  console.error('Usage: node debug_assign_genres.js <board_id> <group_id> <genre_column_id> <comment_column_id>');
  process.exit(1);
}
...
async function assignGenresAndComments() {
  try {
    console.log('Starting to query the board...');
    ...
    const boards = itemQuery.data.boards;
    ...
    if (!items || items.length === 0) {
      console.error('No items found in the specified group.');
      return;
    }
