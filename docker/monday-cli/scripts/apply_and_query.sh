...
query_items() {
    ...
    -H "Authorization: Bearer $MONDAY_TOKEN" \
    -H "Content-Type: application/json" \
    -d "{\"query\": \"query { boards(ids: 2809374317) { groups(ids: \\\"gpt_terminal_plus__1\\\") { items_page(limit: 100) { items { id name column_values(ids: \\\"genre6__1\\\") { text } } } } } }\"}"
